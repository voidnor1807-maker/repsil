...
export interface SyncPlan {
  ...
  /**
   * rel_paths the local side is about to lose (remote content wins). The loser
   * preserves its current copy as a tagged sibling before overwriting — no data
   * loss. Always a subset of pullFiles.
   */
  conflicts: string[]
}
...
export function diffManifests(local: Manifest, remote: Manifest): SyncPlan {
  ...
  for (const r of remote.entries) {
    ...
    if (!l) {
      ...
      // Content differs. Newer mtime wins; ties broken deterministically by hash
      // so both peers agree on the winner. When the remote wins we pull it and
      // flag a conflict so the local (losing) copy is preserved first.
      const remoteWins =
        r.mtime > l.mtime ||
        (r.mtime === l.mtime && (r.content_hash ?? '') > (l.content_hash ?? ''))
      if (remoteWins) {
        pullFiles.push(r.rel_path)
        conflicts.push(r.rel_path)
      }
  ...
  return { pullFiles, pullMeta, deleteLocal, conflicts }
}

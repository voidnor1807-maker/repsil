...
describe('toFtsMatchExpression', () => {
  test('quotes each whitespace-delimited token as a prefix phrase', () => {
  ...
  test('partial token gets prefix wildcard so typing finds matches incrementally', () => {
    expect(toFtsMatchExpression('gi')).toBe('"gi"*')
  })
})
